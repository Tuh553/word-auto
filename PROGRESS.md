...
- `pnpm test`：通过（parser 18/18，validator 67/67，web 13/13；总计 98/98）
- `pnpm -r build`：通过
...
1. parser 深化：继续做脚注尾注、run 级混排；题注 / 交叉引用已完成基础解析与有效性校验。
...
- 当前标准模板未包含正文域样本；题注 / 交叉引用关联回归主要靠 synthetic docx +
  “真实模板 0 field 回归” 双重校验。
