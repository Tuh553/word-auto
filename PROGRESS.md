...
- `pnpm test`：通过（parser 25/25，validator 98/98，web 40/40；总计 163/163）
- `pnpm -r build`：通过
...
1. 表格增强：当前已提取表格段落，但未保留表格与正文的全局交错顺序；表格专属规则/降噪待做。
2. 附录细分：附录内部小标题、成果清单、落款等角色。
3. Web 体验：Web Worker、报告与预览双向滚动、批量检测、带批注 docx 导出。
4. CLI 增强后的回归：继续补 CLI 级测试或 smoke 基线，避免后续参数行为回退。
5. （远期、高风险）自动套版改写——务必无损保留分节、域、题注、交叉引用、编号，绝不引入 Word COM。
