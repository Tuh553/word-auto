...
1. Web 体验：带批注 docx 导出，以及继续打磨报告与预览联动细节（如更细的 run 级局部高亮可视反馈）。
2. 报告复制增强：当前已支持单篇检测结果复制为“修改清单 / 修订建议卡片”MVP，后续可再评估更细粒度模板化输出。
