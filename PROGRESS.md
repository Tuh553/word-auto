...
| `apps/web` | React+Vite 纯前端；四步流程；docx-preview 预览 + 文本匹配高亮（见下，已攻克渲染问题）；规则配置页；字段值编辑；`mode` 切换；草稿保存/发布；发布后回灌检测；多模板切换；自定义规则库 JSON 导入/导出；模板候选面板；**报告项可展开查看规范依据 provenance**；**上传/候选提取错误分流中文提示** |
...
- `pnpm test`：通过（parser 11/11，validator 35/35，web 8/8）
- `pnpm -r build`：通过
...
2. 报告可信度：修复建议、角色识别置信度、问题分组。
3. 模板候选增强：多样本聚合、候选 diff、证据下钻、评分校准。
4. 表格增强：当前已提取表格段落，但未保留表格与正文的全局交错顺序；表格专属规则/降噪待做。
5. Web 体验：Web Worker、默认定位首个问题、报告与预览双向滚动、高亮 run 级区间、批量检测、带批注 docx 导出。
