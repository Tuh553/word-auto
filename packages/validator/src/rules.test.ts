...
const legacy: LegacyRuleLibrary = {
  ...
  page_numbers: { front_matter_format: "RomanUpper", body_format: "Arabic", body_restart_at: 1 },
  headers: { left_text: "测试页眉" },
  styles: {
    body_text: {
      font_east_asia: "宋体",
...
test("normalizeRuleLibrary 旧结构转可编辑结构", () => {
  ...
  assert.equal(lib.document?.paper_size, "A4");
  assert.equal(lib.pageNumbers?.front_matter_format, "RomanUpper");
  assert.equal(lib.headers?.left_text, "测试页眉");
...
test("toLegacyRuleLibrary round-trip 保留核心字段", () => {
  ...
  assert.equal(back.document?.paper_size, "A4");
