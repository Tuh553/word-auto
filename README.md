...
- **主题字体未解析**：run 用 `eastAsiaTheme` 等主题字体时（未显式 `w:eastAsia`），
  字体校验被跳过——需解析 `theme1.xml` 的 `minorFont/majorFont`。
- **行距/缺失项漏报**：段落未显式设置行距时按"继承默认"跳过，未提示缺失。
- **目录条目（TOC1/2/3）暂不校验**。
- **表格内段落、页眉页脚、分节/页码**尚未纳入（第二期）。
