...
apps/web            纯前端工作台：检测预览 + 规则配置 + 多模板 + 模板候选提取
                    React 组件按检测工作区、规则工作区和状态 hooks 拆分
```
...
- 解析：段落文本、样式继承、段落与 run 级有效字体/字号/加粗、对齐/行距/首行缩进/大纲级别
...
- run 级混排检测：段落内局部 run 字体/字号不合规时输出 `startRunIndex` /
  `endRunIndex` / `affectedText`，相邻同值异常 run 合并为区间 issue
...
- web 结构：`App` 只做导航与编排，检测流程、规则库状态和模板候选提取分别收敛到
...
- 工程安全网：`templates/source/*.docx` 金标准解析/分类/校验基线，parser/validator/web
  单测，GitHub Actions 质量门禁覆盖 typecheck、lint、knip、jscpd、test、build

## 剩余 TODO

集中维护在 [`docs/TODO.md`](./docs/TODO.md)。当前重点是角色识别置信度、统计型文档检测、
页眉/页脚样式检测、模板候选质量提升、Web 性能与交付物导出。仍然只做检测，不改写正文；
自动套版属于远期高风险阶段。
