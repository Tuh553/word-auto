...
| 👁️ 浏览器预览 | 前端渲染文档原貌，叠加段落级与片段级不合规高亮，并与报告稳定双向联动 |
...
- 附录内部结构（附录小标题、材料/条目清单、落款或日期署名段落）
...
- ✅ 四步检测流程（上传 → 预览 → 检测 → 报告）
- ✅ Web Worker 检测（解析与校验不阻塞主线程）
- ✅ `docx-preview` 渲染原貌 + 段落文本匹配高亮
- ✅ run 级 issue 片段高亮（优先高亮目标段落内 `affectedText`，找不到或重复命中时回退整段）
- ✅ 报告与预览双向联动（报告点击滚动预览、预览点击反选报告、预览滚动自动切换当前可见 issue；同段多个 issue 按严重级与当前可见顺序稳定选中）
- ✅ 报告结果一键复制（基于当前筛选结果复制“修改清单”，基于当前选中 issue 复制“修订建议卡片”）
- ✅ 规则配置页（字段值编辑 / mode 切换 / 草稿保存 / 发布回灌检测）
- ✅ 多模板管理（切换 / 新建 / 复制 / 重命名 / 删除自定义模板）+ 自定义规则库 JSON 导入/导出
- ✅ 模板候选提取（上传标准模板，聚合角色字段候选值、覆盖率、冲突值、可信提示；支持候选 diff、证据下钻和持久忽略）
