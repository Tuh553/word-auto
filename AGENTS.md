...
- 模板候选当前是单文件增强版：上传 `.docx` -> parser + classify -> 页面设置/角色字段候选 ->
  人工接受到草稿 -> 发布 -> 检测消费新规则。已展示当前草稿、候选 diff（新增/覆盖/启用/一致）、
  冲突值、证据下钻和低置信样本提示；候选忽略状态持久化在本地 UI 状态中，不进入规则库 JSON。
  尚未实现多样本聚合和评分校准。
...
后置部分正式章节、特殊正文元素正式角色、规则配置闭环、多模板管理、模板候选 diff/证据/忽略、报告分组、
规范依据展示、修复建议、Web 预览高亮定位、CLI smoke 路径、CI 质量门禁。

## 下一步（按价值/风险排序）

1. 模板候选增强：多样本聚合与评分校准。
2. 表格增强：保留表格与正文的全局交错顺序，再做表格专属规则/降噪。
3. 附录细分：附录内部小标题、成果清单、落款等角色。
4. Web 体验：Web Worker、报告与预览双向滚动、run 级局部高亮、批量检测、带批注 docx 导出。
5. 远期高风险：自动套版 / 一键修复。只能走纯 OOXML，必须无损保留分节、域、题注、
