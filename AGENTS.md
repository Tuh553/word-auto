...
目录条目、结构化页眉页脚、页眉/页脚样式检测、页眉内容检测、带单位测量值、解析错误分流、
...
1. 统计型文档检测：关键词数量、摘要字数、参考文献条数和外文占比。
2. 角色识别置信度：对低置信启发式命中打标，减少“看似确定”的误导。
3. 多模板管理补齐：新建、复制、重命名、删除模板；内置模板不可误删。
4. 模板候选增强：持久忽略、候选 diff、证据下钻、多样本聚合与评分校准。
5. 表格增强：保留表格与正文的全局交错顺序，再做表格专属规则/降噪。
6. 附录细分：附录内部小标题、成果清单、落款等角色。
7. Web 体验：Web Worker、报告与预览双向滚动、run 级局部高亮、批量检测、带批注 docx 导出。
8. 远期高风险：自动套版 / 一键修复。只能走纯 OOXML，必须无损保留分节、域、题注、
